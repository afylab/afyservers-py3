...
class SR760Wrapper(GPIBDeviceWrapper):
    #TODO
...
class SR760Server(GPIBManagedServer):
    """Serves the Stanford Research Systems SR760 Network Signal Analyzer.
    A low frequency (0-100kHz) FFT. Can save data to datavault."""
    name = 'Signal Analyzer SR760'
    deviceName = 'Stanford_Research_Systems SR760'
    deviceWrapper = SR760Wrapper
    ...
    @setting(20, b='i{averaging mode input}', returns='i{current averaging mode}')
    def average_mode(self, c, b = None):
        """Set the averaging mode to linear (0) or exponential (1). No input queries the current averaging mode."""
        dev = self.selectedDevice(c)
        if b is not None:
            yield dev.write('AVGM' + str(int(b)))
        avgm = yield dev.query('AVGM?')
        returnValue(int(avgm))
    ...
    @setting(26, range='i',returns = 'i')
    def autorange(self, c, range = None):
        """Turns autorange on if range is 1, and off if range is 0. No input queries the current mode."""
        dev = self.selectedDevice(c)
        if range is not None:
            yield dev.write('ARNG%d\n' %range)
        argn = yield dev.query('ARNG?')
        returnValue(int(argn))
    ...
    def power_spectral_amplitude(self, c, trace):
        ...
        avg_mode = yield self.average_mode(c)
        if not avg_mode==0:
            raise Exception('Averaging mode must be set to linear for power spectral amplitude')
