...
\textbf{Buffer ramp (1 ch)} & 131 & \S\ref{sec:bufferramp1} \\
\textbf{Buffer ramp (8 ch)} & 131 & \S\ref{sec:bufferramp8} \\
...
Stop during acquisition & 131 (in-ramp) & \S\ref{sec:stopduring} \\
\midrule
...
\textit{Removed: plain \texttt{buffer\_ramp} \& \texttt{buffer\_ramp\_dis}} & 107, 132 & \textit{No longer present in driver under test} \\
\bottomrule
\end{longtable}
...
DAC0 ramped 0$\to$1~V over 200 steps via \texttt{dac\_led\_buffer\_ramp} while ADC0 (loopback) is read back once per step; the returned trace is compared point-by-point against the ideal linear ramp $V(i) = i/(N-1)$~V ($N=200$) to within 50~mV, and checked for monotonicity & \\
\bottomrule
\end{longtable}

\paragraph{Parameters.} DAC port: [0]; ADC port: [0]; steps: 200; \texttt{dacInterval}: 3000~\textmu s; \texttt{dacSettlingTime}: 500~\textmu s; nReadings: 1. Requires DAC0 jumpered to ADC0.
...
All 8 DAC channels ramped simultaneously via \texttt{dac\_led\_buffer\_ramp} to independent alternating targets ($\pm 2$~V) with \texttt{nReadings=4} (per-step averaging), while all 8 ADC channels (loopback) are read synchronously; each channel's trace is compared point-by-point against its own ideal linear ramp to within 50~mV & \\
\bottomrule
\end{longtable}

\paragraph{Parameters.} DAC ports: [0--7]; ADC ports: [0--7]; steps: 100; \texttt{dacInterval}: 5000~\textmu s (widened to accommodate 8-channel ADC conversion budget); \texttt{dacSettlingTime}: 500~\textmu s; nReadings: 4. Requires DAC$i$ jumpered to ADC$i$ for all $i$.
...
\midrule
\endhead
\seqsplit{test\_time\_series\_buffer\_ramp\_single\_channel} & 1 DAC/1 ADC ramp with independent DAC (1000~\textmu s) and ADC (200~\textmu s) periods; trace compared against the ideal staircase (each of the 100 DAC steps repeated $\mathrm{dacPeriod}/\mathrm{adcPeriod}=5$ times) to within 50~mV & \\
\seqsplit{test\_time\_series\_buffer\_ramp\_eight\_channel} & 8 DAC/8 ADC ramp with independent periods; all 8 channels compared against the same ideal staircase construction & \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
\seqsplit{test\_time\_series\_buffer\_ramp\_2d\_single\_channel} & 1 DAC/1 ADC time-series 2D ramp (3 slow steps $\times$ 50 fast steps, 1:1 DAC/ADC interval); trace compared against the flattened ideal fast/slow grid & \\
\seqsplit{test\_time\_series\_buffer\_ramp\_2d\_eight\_channel} & 8 DAC/8 ADC time-series 2D ramp (3 slow $\times$ 30 fast); all 8 channels compared against their own ideal grid & \\
\seqsplit{test\_dac\_led\_buffer\_ramp\_2d\_single\_channel} & 1 DAC/1 ADC averaged (LED-style) 2D ramp with \texttt{numAdcAverages=2} (3 slow $\times$ 50 fast); trace compared against the ideal fast/slow grid (one averaged point per grid location) & \\
\seqsplit{test\_dac\_led\_buffer\_ramp\_2d\_eight\_channel} & 8 DAC/8 ADC averaged 2D ramp (3 slow $\times$ 30 fast); all 8 channels compared against their own ideal grid & \\
\seqsplit{test\_2d\_ramp\_snake\_and\_retrace\_single\_channel} & Snake $+$ retrace flag combination exercised on minimal (1 ch) case: trace length $\geq$ bare grid size, first point matches \texttt{startPoint}, and alternate rows verified to sweep in opposite fast-axis directions & \\
\bottomrule
\end{longtable}
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Plain \texttt{buffer\_ramp} and \texttt{buffer\_ramp\_dis}} (settings 107, 132) --- removed from the driver under test; superseded by \texttt{dac\_led\_buffer\_ramp} and \texttt{time\_series\_buffer\_ramp}, both of which are covered in \S\ref{sec:bufferramp1}--\S\ref{sec:timeseries}.
